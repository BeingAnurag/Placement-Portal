...
|---|---|---|---|---|
| Authentication/RBAC | Persistent | FastAPI / Prisma fallback | Password-only sign-in (`/register`, `/account/password`, admin set-password, `npm run db:set-password`), 5-tier role hierarchy (`STUDENT`, `COORDINATOR`, `OFFICER`, `ADMIN`, `SUPER_ADMIN`), granular 17-permission RBAC catalog, per-user custom permission matrix, user provisioning, elevation/de-elevation, emergency `ADMIN_EMAILS` bootstrap. Every admin route is registered in `ROUTE_PERMISSIONS`, which both the middleware and the admin sidebar read, so the navigation only offers pages the account can open | Verify production callback/cookies and deployment secrets; add mailbox verification and a forgot-password flow before external exposure |
...
| Interview Experiences | Persistent | FastAPI | Student submission (company, role, batch, interview type, per-category question fields) held `PENDING` until reviewed; student browse of `APPROVED` submissions and own-submission status tracking; admin moderation (`/admin/interview-experiences`) with approve/reject/delete guarded by `interview_experiences:manage`, student notification and email dispatch on review | Add per-submission edit for admins, and richer full-text search/filtering |
